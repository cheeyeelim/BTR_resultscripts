...
for(acyclic in c('acyclic', 'cyclic', 'both'))
{
  for(single_cell in c(T, F))
  {
    ...
    if(acyclic=='acyclic')
    {
      files = list.files(path=path, pattern=sprintf('^result.+_train_acyclic_%s_yeast[.]rda', ifelse(single_cell, 'sc', 'nonsc')), full.names = T)
    } else if(acyclic=='cyclic')
    {
      files = list.files(path=path, pattern=sprintf('^result.+_train_cyclic_%s_yeast[.]rda', ifelse(single_cell, 'sc', 'nonsc')), full.names = T)
    } else if(acyclic=='both')
    {
      files = list.files(path=path, pattern=sprintf('^result.+_train_[a-z]+_%s_yeast[.]rda', ifelse(single_cell, 'sc', 'nonsc')), full.names = T)
    }
    ...
    for(i in 1:length(files))
    {
      if(acyclic=='acyclic')
      {
        tmp_name = gsub(sprintf('.+result_(.+)_train_acyclic_%s_yeast[.]rda', ifelse(single_cell, 'sc', 'nonsc')), '\\1', files[i]) #extract name of method.
      } else if(acyclic=='cyclic')
      {
        tmp_name = gsub(sprintf('.+result_(.+)_train_cyclic_%s_yeast[.]rda', ifelse(single_cell, 'sc', 'nonsc')), '\\1', files[i]) #extract name of method.
      } else if(acyclic=='both')
      {
        tmp_name = gsub(sprintf('.+result_(.+)_train_.+_%s_yeast[.]rda', ifelse(single_cell, 'sc', 'nonsc')), '\\1', files[i]) #extract name of method.
      }
    ...
    comb_p1 = ggplot(plot_df, aes(x=plot_df[,'method_name'], y=plot_df[,"value"])) +
      ...
      xlab('Inference algorithms') + ylab('') + ggtitle(Hmisc:::capitalize(acyclic)) +
      facet_wrap(~variable, scales='free_y', ncol=2)
    
    png(paste('all_algorithms_', acyclic, ifelse(single_cell, '_sc', '_nonsc'), '_score.png', sep=''), width=3000, height=4000, res=300)
    plot(comb_p1)
    dev.off()
    ...
    png(paste('all_algorithms_', acyclic, ifelse(single_cell, '_sc', '_nonsc'), '_meanscore.png', sep=''), width=3000, height=4000, res=300)
    plot(combmid_p1)
    dev.off()
    ...
    comb_p8 = ggplot(comb_df, aes(x=reorder(comb_df[,'method_name'], -comb_df[,"F-score"], FUN=median), y=comb_df[,"F-score"])) +
      geom_boxplot() + geom_jitter() +
      theme(axis.text.x = element_text(angle = 90, hjust = 1), text = element_text(size=20)) + 
      xlab('Inference algorithms') + ylab('F-score') + ggtitle(Hmisc:::capitalize(acyclic))
    
    png(paste('all_algorithms_', acyclic, ifelse(single_cell, '_sc', '_nonsc'), '_fscore.png', sep=''), width=2000, height=2000, res=300)
    plot(comb_p8)
    dev.off()
    ...
    png(paste('all_algorithms_', acyclic, ifelse(single_cell, '_sc', '_nonsc'), '_fmeanscore.png', sep=''), width=2000, height=2000, res=300)
    plot(combmid_p8)
    dev.off()
  }
}
